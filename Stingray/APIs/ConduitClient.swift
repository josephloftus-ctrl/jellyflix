...
private let logger = Logger(subsystem: "com.benlab.stingray", category: "conduit")

/// A single recommendation row from Conduit
struct RecommendationRow: Codable, Sendable {
    let title: String
    let reason: String
    let itemIds: [String]
    let type: String
...
final class ConduitClient: @unchecked Sendable {
    let baseURL: URL
    ...
    func getRecommendations(userId: String) async throws -> RecommendationsResponse {
        ...
        do {
            ...
            logger.error("Conduit request failed: \(error.localizedDescription)")
            throw ConduitError.requestFailed(error.localizedDescription)
        }

        do {
            return try JSONDecoder().decode(RecommendationsResponse.self, from: data)
        } catch {
            logger.error("Conduit decode failed: \(error.localizedDescription)")
            throw ConduitError.decodeFailed(error.localizedDescription)
        }
    }
}
